...
// RICE = (reach × impact × confidence%) / effort; для done/cancelled — null (приоритет мёртвой задачи не нужен)
const rice = (t) => {
    ...
    for (const slug of readdirSync(projectsDir)) {
      ...
      if (existsSync(tdir)) {
        for (const f of readdirSync(tdir)) {
          ...
          // имя папки — источник правды для проекта (frontmatter project — wiki-ссылка, не slug)
...
// Obsidian-ссылки [[путь|label]] / [[путь]] → читаемый текст
...
function render({ projects, tasks, inbox }) {
  ...
  const kcols = {
    todo: byRice.filter((t) => t.status === "todo" || t.status === "blocked"),
    doing: open.filter((t) => t.status === "doing"),
    done: tasks.filter((t) => t.status === "done").sort((a, b) => String(b.updated).localeCompare(String(a.updated))),
  };
  const doneShown = kcols.done.slice(0, 15);
  const maxRice = byRice[0]?._rice || 1;
  ...
  const kanbanCol = (title, list, color, extra = "") => `<div class="kcol">
    <div class="kcol-h"><span class="kpill" style="background:${color.bg};color:${color.fg}">${title}</span><span class="kcount">${list.length}</span></div>
    ${list.map(taskCard).join("") || '<div class="kempty">пусто</div>'}
    ${extra}
  </div>`;

  const riceRows = byRice.slice(0, 40).map((t) => `<tr>
      <td class="kc-id-cell">${ticket(t.id)}</td>
      <td class="tname">${sumtext(t)}</td>
      <td class="muted">${esc(t.project)}</td>
      <td class="ricecell">
        <span class="rice-bar"><span style="width:${Math.round(((t._rice ?? 0) / maxRice) * 100)}%"></span></span>
        <span class="rice">${fmtRice(t._rice)}</span>
      </td>
    </tr>`).join("");

  const inboxBlock = inbox.length ? `<section class="block" id="inbox">
      <h2 class="sec-h">Инбокс · ${inbox.length}</h2>
    ...
    </section>` : `<section class="block" id="inbox"><h2 class="sec-h">Инбокс · 0</h2><p class="empty-box">Инбокс пуст</p></section>`;
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .nav-in { max-width:1180px; margin:0 auto; display:flex; align-items:center; gap:18px; padding:12px 20px; }
  ...
  .nav a.link { color:var(--muted); font-size:14px; padding:6px 10px; border-radius:8px; }
  .nav a.link:hover { background:#f5f5f5; color:var(--ink); }
  ...
  .card { display:flex; flex-direction:column; background:#fff; border:1px solid var(--line); border-radius:14px; box-shadow:0 1px 2px rgba(0,0,0,.04); padding:16px; transition:.15s; }
  .card:hover { border-color:#d4d4d4; box-shadow:0 4px 12px rgba(0,0,0,.06); }
  ...
  .card-top { margin-top:14px; padding-top:12px; border-top:1px solid #f5f5f5; font-size:14px; display:flex; align-items:center; justify-content:space-between; gap:8px; }
  .ct-sum { color:#525252; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  ...
  .badge { display:inline-flex; align-items:center; font-size:11px; font-weight:500; padding:2px 9px; border-radius:999px; line-height:1.5; }
  ...
  .kcol { background:rgba(245,245,245,.55); border:1px solid var(--line); border-radius:14px; padding:12px; }
  ...
  .kc { background:#fff; border:1px solid var(--line); border-radius:10px; box-shadow:0 1px 2px rgba(0,0,0,.04); padding:10px 11px; margin-bottom:8px; }
  ...
  .kc-sum { font-size:14px; font-weight:500; color:#262626; margin-top:3px; }
  .kc-foot { display:flex; gap:6px; margin-top:8px; }
  ...
  .block table { width:100%; border-collapse:collapse; background:#fff; border:1px solid var(--line); border-radius:14px; overflow:hidden; }
  ...
  .kc-id-cell { font-family:ui-monospace,monospace; font-size:11px; color:#a3a3a3; width:54px; }
  .tname { font-weight:500; }
  .ricecell { display:flex; align-items:center; justify-content:flex-end; gap:8px; width:140px; }
  .rice-bar { height:6px; width:60px; border-radius:999px; background:#f5f5f5; overflow:hidden; }
  .rice-bar span { display:block; height:100%; background:var(--accent); border-radius:999px; }
  .inbox { display:grid; gap:8px; max-width:680px; }
  ...
  .empty-box { border:1px dashed var(--line); border-radius:14px; padding:30px; text-align:center; color:#a3a3a3; font-size:14px; }
  .hint { color:#a3a3a3; font-size:12px; margin:12px 0 0; }
  @media (max-width:860px){ .grid,.kanban{grid-template-columns:1fr 1fr} }
  @media (max-width:560px){ .grid,.kanban{grid-template-columns:1fr} .nav a.link{padding:6px 7px} }
</style></head><body>
<nav class="nav"><div class="nav-in">
  <span class="logo"><i>ll</i> taskvault</span>
  <a class="link" href="#proj">Проекты</a>
  <a class="link" href="#kanban">Канбан</a>
  <a class="link" href="#rice">RICE</a>
  <a class="link" href="#inbox">Инбокс</a>
</div></nav>
<div class="wrap">
  <h1>Проекты</h1>
  <p class="stats">${projects.length} проектов · ${open.length} открытых задач${inbox.length ? ` · инбокс ${inbox.length}` : ""}</p>

  <div id="proj"></div>
  ${active.length ? section("Активные", active.length, "#10b981", active.map((p) => projectCard(p, tasks)).join("")) : ""}
  ${others.length ? section("Остальные <span style='font-weight:400;text-transform:none;color:#d4d4d4'>готовы · пауза · идеи</span>", others.length, "#d4d4d4", others.map((p) => projectCard(p, tasks)).join("")) : ""}

  <section class="block" id="kanban">
    <h2 class="sec-h">Канбан</h2>
    <div class="kanban">
      ${kanbanCol("Todo", kcols.todo, { bg: "#f1f5f9", fg: "#475569" })}
      ${kanbanCol("В работе", kcols.doing, { bg: "#eff6ff", fg: "#1d4ed8" })}
      ${kanbanCol("Готово", doneShown, { bg: "#ecfdf5", fg: "#047857" }, kcols.done.length > doneShown.length ? `<div class="kmore">+ ещё ${kcols.done.length - doneShown.length} (показаны 15 свежих)</div>` : "")}
    </div>
  </section>

  <section class="block" id="rice">
    <h2 class="sec-h">RICE-приоритеты · ${byRice.length}</h2>
    <table><thead><tr><th>#</th><th>Задача</th><th>Проект</th><th style="text-align:right">RICE</th></tr></thead>
    <tbody>${riceRows || '<tr><td colspan="4" class="muted">Открытых задач нет.</td></tr>'}</tbody></table>
  </section>

  ${inboxBlock}

  <p class="hint">Обновляется автоматически каждые 5 c. Данные читаются из markdown при каждом запросе — правки агента видны сразу.</p>
</div>
